...
mono_socio %>% filter(QuestionCategory=="sex") %>% group_by(Responce) %>% count

mono_socio %>% filter(QuestionCategory=="education") %>% group_by(Responce) %>% count()

mono_socio %>% filter(QuestionCategory=="handedness") %>% group_by(Responce) %>% count()
```
...
```{r}
library(ggplot2)
qplot(mono$RT, binwidth=100, xlab="reaction time", ylab="frequency")
...
```{r}
qplot(mono$RTlog, binwidth=0.01, xlab="log reaction time", ylab="frequency")
```
...
```{r}
mono1 %>%
  group_by(WordOrder, TypeOfStimuli, AcceptabilityJ) %>% count() %>%
  ggplot(aes(x=AcceptabilityJ, y=n)) + 
    geom_bar(stat="identity") +
    facet_wrap( ~ WordOrder + TypeOfStimuli, ncol=6)
```
...
ggline(mono1, x="AcceptabilityJ", y="RTlog",
       linetype = "WordOrder", color="WordOrder",
       add="mean_se",
       error.plot = "errorbar")
```
...
```
We must recject the hypothesis that participants spend more time on
incongruent word orders.
```
...
t.test(x=mean_rtlog$diff, alternative="greater")
```
...
```
Individuals take significantly longer to respond to incongruent word orders.
```

How can you interpret difference with the result of 2.3?
```
The variation between participants added noise to data and thus
...
```{r}
t.test(x = incong_rtlog$RTlog[incong_rtlog$TypeOfStimuli == "Shape-Color"],
       y = incong_rtlog$RTlog[incong_rtlog$TypeOfStimuli == "SubjectiveComment-Material"],
       alternative="two.sided")
```
```
SubjectiveComment-Material can be answered faster than Shape-Color.
```
...
```{r}
fm1 <- aov(RTlog ~ TypeOfStimuli, data=incong_rtlog)
TukeyHSD(fm1)
```

```
SubjectiveComment-Material-Shape-Color is the only stimulus with
a significant deviation in response time.
```
### R code cookbook
